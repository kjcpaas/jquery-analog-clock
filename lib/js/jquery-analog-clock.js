  ...
  AnalogClock.prototype = {
    ...
    _attachHandlers: function() {
      if(this.options.moveable) {
        ...
        this.clock.on('mousedown', '.hour-hand', this._dragHourHand.bind(this));
      }
    },

    _dragHourHand: function(e) {
      this.clock.on('mousemove', function(e) {
        var currentCursorAngle = this._currentCursorAngle(e);
        var angleDifference = this._correctAngleDifference(currentCursorAngle - this._currentHourAngle());
        // Correct angle difference when crossing 12 o'clock
        var newMinute = this._currentMinute() + Math.round(angleDifference / 0.5);
    ...
    _currentHourAngle: function() {
      return (this._currentHour() % 12) * 360 / 12 + this._currentMinute() * 360 / (60 * 12);
    },
    ...
    _correctAngleDifference: function(angleDiff) {
      // Correct angle difference when crossing 12 o'clock direction when dragging hands
